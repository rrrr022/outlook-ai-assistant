...
async function generateWord(title: string, content: string, options: DocumentOptions = {}): Promise<GeneratedDocument> {
  const { Document, Packer, Paragraph, TextRun, HeadingLevel, Table, TableRow, TableCell, WidthType, BorderStyle, AlignmentType, Header, Footer, PageNumber } = await loadDocx();
  ...
  // Footer
  children.push(new Paragraph({
    children: [new TextRun({ text: `${TEMPLATES[template].icon} Generated by Outlook AI - FreedomForged_AI`, italics: true, size: 18, color: '888888' })],
    spacing: { before: 600 },
    alignment: AlignmentType.CENTER,
  }));
...
<div class="header">
  ...
  html += `<div class="footer">
    ${TEMPLATES[template].icon} Generated by <strong>Outlook AI - FreedomForged_AI</strong><br>
...
/**
 * Generate Excel (.xlsx) with charts support
 */
